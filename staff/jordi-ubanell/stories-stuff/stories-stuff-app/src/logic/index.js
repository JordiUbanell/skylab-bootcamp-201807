...
const logic = {
    ...
    //  * Set / Get userName
    //  * 
    //  * @example - logic.retrieveUser() // Returns the token
    //  * @example - logic._userName(name) // Saves the token
    //  * 
    //  * @param {*} userName 
    //  */
  ...
  _retrieveUser() {
       return Promise.resolve()
           .then(() => {
            ...
            return this._callApiStory(`user/${this._userEmail()}`,'GET',undefined, { authorization: `bearer ${this._userToken()}`},201)
           })
           .then(res => res.data)
   },
     ...
     * Set / Get userName
     * 
     * @example - logic._userName() // Returns the token
     * @example - logic._userName(name) // Saves the token
     * 
     * @param {*} userName 
     */
    ...
    _callApiStory(path, method = 'get', body, token, expectedStatus = 200) {
        ...
        return fetch('https://powerful-thicket-52138.herokuapp.com/api/' + path, config)
            .then(res => {
    ...
    updateProfile(password,newName,newSurname,newPassword){

        return this._callApiStory(`user/${this._userEmail()}/update`, 'PATCH',
        {
            newName,
            newSurname,
            newPassword,
    ...
    updateStory(text, productId, storyId) {
        return Promise.resolve().then(() => {
    ...
    deleteStory(storyId) {
        return Promise.resolve().then(() =>
        this._callApiStory(`user/${this._userEmail()}/stories/${storyId}`,'DELETE',undefined,{
            authorization: `bearer ${this._userToken()}`, 'content-type': 'application/json'
        }, 200)
        .then(() => true )
        )
    ...
    searchWord(word) {
        return Promise.resolve().then(() => {
           return this._callApiStory(`search/${word}`).then(result => result)
        })
    },
    ...
    AddLikeToStory(productId, storyId) {
        
        return Promise.resolve().then(() => {
            
            return this._callApiStory(`/user/${this._userEmail()}/products/${productId}/stories/${storyId}/like`, 'POST',
            undefined,
            {
                authorization: `bearer ${this._userToken()}`, 'content-type': 'application/json'
            }, 201)
